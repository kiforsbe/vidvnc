...
test('desktop owner receives onboarding data and can shut down its server', async () => {
  const child = spawn(
    process.execPath,
    [fileURLToPath(new URL('../../apps/server/src/main.mjs', import.meta.url)), '--desktop'],
    {
      cwd: new URL('.', import.meta.url),
      env: { ...process.env, VIDVNC_PORT: '4389', VIDVNC_HOST: '127.0.0.1' },
      windowsHide: true,
    },
  );
  ...
  assert.match(ready.password, /^[A-Z]{4}-[A-Z]{4}$/);
