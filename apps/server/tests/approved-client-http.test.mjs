...
test('HTTP registration waits for host approval and releases the credential only to its claimant', () =>
  withServer(async (url, server, approvedClients) => {
    ...
    assert.deepEqual(
      await (
        await post(url, 'approved-clients/status', registration)
      ).json(),
      { state: 'pending' },
    );
...
test('one-time connection keys admit exactly one ordinary session and are then deleted', () =>
  withServer(async (url, server) => {
    ...
    const occupied = await post(url, 'connect', { password: server.sessionStore.password });
    const occupiedSession = await occupied.json();
    ...
    await post(url, 'disconnect', {}, occupiedSession.sessionId);
