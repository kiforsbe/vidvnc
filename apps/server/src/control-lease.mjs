...
export class ControlLease {
  ...
  grant(sessionId, streamId, { onlyIfAvailable = false } = {}) {
    ...
    return this.#serialize(async () => {
      ...
      if (!this.isActive(sessionId, streamId)) throw new Error('Inactive control target');
      await this.#release();
      if (!this.isActive(sessionId, streamId)) throw new Error('Inactive control target');
      try {
        if ((await this.media.setPermission(streamId, true)) !== true)
          throw new Error('Worker refused control grant');
        this.#owner = { sessionId, streamId };
        if (!this.isActive(sessionId, streamId)) {
          await this.#release();
  ...
  revoke(sessionId) {
    return this.#serialize(async () => {
      if (sessionId === undefined || this.#owner?.sessionId === sessionId) await this.#release();
    });
  }
  renew() {
    ...
    return this.#serialize(async () => {
      ...
      if (!owner) return;
      if (!this.isActive(owner.sessionId, owner.streamId)) return this.#release();
      try {
        if ((await this.media.setPermission(owner.streamId, true)) !== true)
          throw new Error('Worker refused control renewal');
        if (!this.isActive(owner.sessionId, owner.streamId)) await this.#release();
      } catch {
        await this.#release();
      }
    });
