...
export function createViewer({ onExit }) {
  ...
  async function disconnect(
    ...
    if (notify) await onExit(message);
  }
  ...
  async function startStream(result, attempt) {
    ...
    assertCurrent();
    $('viewerName').textContent = catalog.serverName;
  ...
  window.addEventListener('pagehide', () => disconnect(undefined, false), {
    signal: events.signal,
  });
  return {
    start(result) {
      if (disposed) throw new Error('Connection cancelled.');
      if (!window.RTCPeerConnection) throw new Error('This browser does not support WebRTC.');
      return startStream(result, ++connectionAttempt);
    },
    disconnect: (message) => disconnect(message, false),
  };
}
