...
## 2026-09-21: what multi-vendor encoding changed about these numbers

These measurements stand, but they are now measurements of **one backend**. Multi-vendor encoding (see [the design](../superpowers/specs/2026-09-21-hardware-encoders-design.md)) changed how the floors above are expressed and, on other hardware, what they are worth.

- **The floors are the same values, stored differently.** `qp-min-i`/`qp-min-p` is NVENC's spelling, and 0 to 51 (H.264/H.265) and 0 to 255 (AV1) are the ranges NVENC declares. The tuned numbers above are now stored as a fraction of the property's declared range and rescaled onto whatever range the target element reports, so the same quality tier means the same thing on an encoder with a different scale. Rescaling onto NVENC's own ranges reproduces the literals above exactly, and a regression test asserts the emitted property string is byte-for-byte unchanged. **Nothing about NVENC's behaviour changed.**
- **Property names are not portable, and the floors are not always available.** Each family spells the floors differently, and AMF differs across its own codecs: `amfh264enc` has a single global `min-qp`, `amfh265enc` has `min-qp-i`/`min-qp-p`, and `amfav1enc` has no floor property at all. Where an element declares no floor, it is dropped and logged. A dropped floor costs the saving measured above — that stream behaves closer to CBR — but cannot breach `max-bitrate`, so the failure mode is spending too much, not overrunning the cap.
- **The savings figures have not been re-measured on other hardware.** The 93%/89%/78% results are NVENC on an RTX 5060 Ti. AMF and Media Foundation were verified to encode correctly and produce watchable streams, but their VBR savings were not quantified. Quick Sync was never run at all.
- **What would close the gap.** Repeating the per-codec floor sweep above on an Intel machine and on AMD, and in particular measuring latency on Quick Sync, which is the one family with no low-latency or target-usage property — its latency comes only from CBR, one reference frame, no B-frames and a short GOP.
