...
**Goal:** Implement approved-client setup, host approval, durable credentials, browser-secret storage, and username/password sign-in end to end while retaining ordinary one-time connections.
...
- [ ] Write tests for all purpose-bit patterns, exact lookup, expiry, multi-use Session admission, atomic deletion of both single-use types, persistence redaction, claim isolation, approval/rejection, three-part authentication, and removal.
- [ ] Run the new tests and confirm failures are missing-module/missing-method failures.
- [ ] Implement the key registry, approved-client store, session admission seam, and settings path.
- [ ] Run the new tests and existing session-store tests until green.
...
- [ ] Write HTTP tests proving flow separation, generic invalid responses, no secret leakage, host approval completion, and approved sign-in.
- [ ] Run the HTTP tests and confirm the routes fail before implementation.
- [ ] Add the routes and factor common post-admission session setup without changing existing `/api/connect` behavior.
- [ ] Run approved-client HTTP, HTTP security, session, and client-settings tests until green.
...
- [ ] Extend the owner fixture and Navigation test to require real setup-key presentation and correctly addressed client commands.
- [ ] Run the Navigation regression and confirm failure against disabled controls.
- [ ] Wire persistent store startup, status output, setup creation, approval/rejection/removal, and host request/reply handling.
- [ ] Run the Navigation regression and production host build until green.
...
**Interfaces:**
- Origin-scoped IndexedDB record `{ clientId, clientSecret, username, deviceName }`.
- UI modes: `connection-key`, `register`, `pending`, and `sign-in`; one-time connection remains the fallback.

- [ ] Write unit/browser assertions for saved-credential validation, mode choice, setup registration, pending approval, credential storage, username/password sign-in, and connection-key fallback.
- [ ] Run the tests and confirm failure before adding the new module/UI.
- [ ] Implement IndexedDB storage and the four UI states, reusing existing stream startup after either admission route.
- [ ] Run web-client unit tests, approved-client HTTP tests, and the browser fixture until green.
...
- [ ] Run formatter checks, server/web unit suites excluding the known Node 26 virtual-terminal baseline, Navigation regression, production host build, and `git diff --check`.
- [ ] Confirm persisted test fixtures contain no plaintext password or client secret and all HTTP/owner payloads redact secrets from host status.
